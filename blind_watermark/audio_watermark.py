...
def lsb_watermark(cover_filepath, watermark_data, watermarked_output_path):
    ...
    if len(samples) < len(watermark_bits):
        raise OverflowError(
            "The watermark data provided is too big to fit into the cover audio! Tried to fit %d bits into %d bits of space." % (
            len(watermark_bits), len(samples)))
...
if __name__ == "__main__":
    cover_audio = "../examples/audio/ori.wav"
    output = "../examples/output/w.wav"
    if len(sys.argv) > 1:
        message = sys.argv[1]
        if len(sys.argv) > 2:
    ...
    recover_embedded_file('../examples/output/w.wav', '../examples/audio/')